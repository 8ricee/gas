 ...
 */
function buildListValidation(listValues) {
...
function applyValidationToColumn(sheet, colIndex, rule) {
...
function setColumnListValidation(sheet, colIndex, listValues) {
  applyValidationToColumn(sheet, colIndex, buildListValidation(listValues));
}
...
function _setupNhanVienValidations(ss, chiNhanhRule) {
  ...
  setColumnListValidation(nvSheet, COL_NV.VAI_TRO, EMPLOYEE_ROLES);
  setColumnListValidation(nvSheet, COL_NV.QUYEN_XUAT, [YES_NO_VALUES.YES, YES_NO_VALUES.NO]);
  setColumnListValidation(nvSheet, COL_NV.TRANG_THAI, [EMPLOYEE_STATUS.ACTIVE, EMPLOYEE_STATUS.INACTIVE]);
}
...
function _setupDonHangValidations(ss, chiNhanhRule) {
  ...
  setColumnListValidation(dhSheet, COL_DH.HINH_THUC_BAN, [SALES_METHOD.DIRECT, SALES_METHOD.INSTALLMENT]);
  ...
  setColumnListValidation(dhSheet, COL_DH.CO_NHAN_QUA, [YES_NO_VALUES.YES, YES_NO_VALUES.NO]);
...
function _setupDichVuValidations(ss, chiNhanhRule) {
  ...
  setColumnListValidation(dvSheet, COL_DV.LOAI_DV, [SERVICE_TYPES.TRANSFER_HO, SERVICE_TYPES.CASH_WITHDRAWAL, SERVICE_TYPES.PHONE_TOPUP]);
...
function _setupTraGopValidations(ss, chiNhanhRule) {
  ...
  setColumnListValidation(tgSheet, COL_TG.TRANG_THAI, [INSTALLMENT_STATUS.RUNNING, INSTALLMENT_STATUS.DONE, INSTALLMENT_STATUS.LATE, INSTALLMENT_STATUS.CANCELLED, INSTALLMENT_STATUS.PENDING]);
...
function _setupDoiTraValidations(ss, chiNhanhRule) {
  ...
  setColumnListValidation(doiTraSheet, COL_DT_TRA.LOAI_GD, [
    EXCHANGE_TYPES.RETURN_DEVICE,
    EXCHANGE_TYPES.EXCHANGE_DEVICE,
    EXCHANGE_TYPES.RETURN_GOODS,
    EXCHANGE_TYPES.EXCHANGE_GOODS
  ]);
...
function _setupThuMuaValidations(ss, chiNhanhRule, thuongHieuRule) {
  ...
  setColumnListValidation(thuMuaSheet, COL_TM.LOAI_GD, [SALES_METHOD.DIRECT, "Thu cũ đổi mới"]);
  setColumnListValidation(thuMuaSheet, COL_TM.HINH_THUC_TT, [PAYMENT_METHOD.CASH, PAYMENT_METHOD.TRANSFER, PAYMENT_METHOD.POS, SALES_METHOD.INSTALLMENT, PAYMENT_METHOD.MIXED, "Trừ vào đơn mới"]);
  ...
  if (nvSheet) {
    ...
    if (lastRow > 1) {
      ...
      nvData.forEach(function (row) {
        ...
        if (maNV && trangThai !== EMPLOYEE_STATUS.INACTIVE) {
